...
interface ShadowStats {
  ...
  chillingRendDamage: string;
}

@Component({
  selector: 'app-shadowspawn',
  templateUrl: 'shadowspawn.page.html',
  styleUrls: ['shadowspawn.page.scss'],
...
export class ShadowspawnPage implements OnInit {
  selectedLevel = 3;
  selectedType: string = 'Fury';
  stats: ShadowStats | null = null;
  currentHP = 0;
  maxHP = 0;
  isHPModalOpen = false;

  levels: number[] = [3, 4, 5, 6, 7, 8, 9];
  types: string[] = ['Fury', 'Despair', 'Fear'];

  private spellId = 'shadowspawn';
  ...
  private updateStats(): void {
    ...
    const hp = this.selectedLevel * 15 - 10;
    const multiattackCount = Math.max(1, Math.floor(this.selectedLevel / 2));
    const chillingRendDamage = `1d12 + ${3 + this.selectedLevel} cold`;

    this.stats = { ac, hp, multiattackCount, chillingRendDamage };
  }
