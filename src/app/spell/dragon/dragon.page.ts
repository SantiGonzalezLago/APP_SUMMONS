...
export class DragonPage implements OnInit {
  selectedLevel = 5;
  selectedType: string = 'Acid';
  stats: DragonStats | null = null;
  currentHP = 0;
  maxHP = 0;
  isHPModalOpen = false;

  levels: number[] = [5, 6, 7, 8, 9];
  types: string[] = ['Acid', 'Cold', 'Fire', 'Lightning', 'Poison'];

  private spellId = 'dragon';
  ...
  private updateStats(): void {
    const ac = 14 + this.selectedLevel;
    const hp = this.selectedLevel * 10;
    const multiattackCount = Math.max(1, Math.floor(this.selectedLevel / 2));
    const rendDamage = `1d6 + ${4 + this.selectedLevel}`;

    this.stats = { ac, hp, multiattackCount, rendDamage };
  }
  ...
  getDamageTypeLowercase(): string {
    return this.selectedType.toLowerCase();
  }
