...
export class BeastPage implements OnInit {
  ...
  stats: BeastStats | null = null;
  currentHP = 0;
  maxHP = 0;
  isHPModalOpen = false;

  levels: number[] = [2, 3, 4, 5, 6, 7, 8, 9];
  types: string[] = ['Air', 'Land', 'Water'];

  private spellId = 'beast';
  ...
  private updateStats(): void {
    ...
    const baseHP = this.selectedType === 'Air' ? 10 : 20;
    const hp = baseHP + 5 * this.selectedLevel;
    const multiattackCount = Math.max(1, Math.floor(this.selectedLevel / 2));
    const rendDamage = `1d8 + ${4 + this.selectedLevel}`;
