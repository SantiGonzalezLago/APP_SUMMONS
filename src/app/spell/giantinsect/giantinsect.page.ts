...
interface InsectStats {
  ac: number;
  hp: number;
  multiattackCount: number;
  poisonJabDamage: string;
  webBoltDamage: string;
}

@Component({
  selector: 'app-giantinsect',
  templateUrl: 'giantinsect.page.html',
  styleUrls: ['giantinsect.page.scss'],
...
export class GiantInsectPage implements OnInit {
  selectedLevel = 4;
  selectedType: string = 'Spider';
  stats: InsectStats | null = null;
  currentHP = 0;
  maxHP = 0;
  isHPModalOpen = false;

  levels: number[] = [4, 5, 6, 7, 8, 9];
  types: string[] = ['Spider', 'Wasp', 'Centipede'];

  private spellId = 'giantinsect';
  ...
  private updateStats(): void {
    ...
    const poisonJabDamage = `1d6 + ${3 + this.selectedLevel} piercing plus 1d4 poison`;
    const webBoltDamage = `1d10 + ${3 + this.selectedLevel} bludgeoning`;

    this.stats = { ac, hp, multiattackCount, poisonJabDamage, webBoltDamage };
  }
